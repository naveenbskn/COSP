...
class VideoPlayer(ttk.Frame):
    ...
    def set_setup(self, prop: dict)->dict:
        ...
        default = {'play':  True, 'camera': False, 'pause': True, 'stop': False, 'image': False,'braille': True,'deaf': True, 'algo': False}
        setup = default.copy()
        setup.update(prop)
    ...
    def _build_widget(self, parent: ttk.Frame=None, setup: dict=dict):
        ...
        if parent is None:
            self.master.geometry("2560x1440+0+0")
            p1 = PhotoImage(file = 'logo.png')
            self.master.iconphoto(False, p1)
            self.master.title("C O S P")
        ...
        if setup['deaf']:
            ...
            self.icon_image = PhotoImage(file=os.path.join(icons_path, 'deaf.PNG'))
            button_load_image = Button(control_frame, padx=10, pady=10, bd=8, fg="white", font=('arial', 12, 'bold'),
                                       ...
                                       height=icon_height, width=icon_width,
                                       command=lambda: self.deaf())
            button_load_image.pack(side='left')
        if setup['braille']:
            # stop video button button_live_video
            self.icon_stop = PhotoImage(file=os.path.join(icons_path, 'braille.PNG'))
            button_stop_video = Button(control_frame, padx=10, pady=10, bd=8, fg="white", font=('arial', 12, 'bold'),
                                       text="braille", bg='black', height=icon_height, width=icon_width,
                                       image=self.icon_stop,
                                       command=lambda: self.braille())
            button_stop_video.pack(side='left')
        ...
        while self.__cap.isOpened():
            ...
            if self.__play:
                ...
                if ret:
                    ...
                    #save frame for color detection
                    im=self.frame.save("text.jpg")
    ...
    def braille(self):
        files = [('Word Document', '*.doc'), 
                ('Text Document', '*.txt')] 
        frr = asksaveasfile(filetypes = files, defaultextension = files) 
        frr=str(frr)
        #print(frr)
        if frr !="None":
            f=open("temp.txt","w")
            ...
            self.master.destroy()
            import braille_con
            braille_con.call(movie_filename)
            os.system("main.exe")
    ...
    def deaf(self):
        ...
        if frr !="None":
            ...
            self.master.destroy()
            import deaf_con
            deaf_con.call(movie_filename)

            os.system("main.exe")
        
        #if te=="completed":
            #messagebox.showerror("COSP", "Document has been generated sucessfully!!")
...
def main():
    vid = VideoPlayer(image=True, play=True, camera=True, algo=False)
